...
COPY ${SLURM_WORKDIR:-.}/scripts/signal-override.sh /tmp/signal-override.sh
# copy slurm plugin scripts
...
RUN echo '/usr/lib/jarvice.slurm/scripts/start-slurm.sh &> /dev/null &' >> /etc/profile
# install dependencies
COPY ${SLURM_WORKDIR:-.}/buildscripts/install-dependencies.sh /usr/local/bin/install-dependencies.sh
RUN  /usr/local/bin/install-dependencies.sh
# install slurm
...
# setup AppDef
ARG APPDEF
COPY ${APPDEF:-${SLURM_WORKDIR:-.}/NAE/AppDef.json} /etc/NAE/AppDef.json
